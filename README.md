...
`sbctl` 不包含 Cloudflare DNS 自动验证逻辑。
...
- 备份包含托管证书副本，但不包含 Certbot 账户/lineage；恢复后若缺少续期数据，会明确提示重新签发。
...
CI 会安装当前稳定 sing-box，并使用真实 `sing-box check` 运行协议、旧配置迁移、REALITY/Hysteria2、证书生命周期和卸载安全测试。

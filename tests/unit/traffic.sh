...
SBCTL_TESTING=1 \
SBCTL_TRAFFIC_NOW='2026-08-24 12:00:00' \
...
[[ $vless_line -lt $socks_line && $socks_line -lt $hy2_line ]]
...
# Entries before the current period are removed, while the current period remains queryable.
tmp=$(temp_file)
...
SBCTL_TRAFFIC_TODAY=2026-08-24 \
SBCTL_TRAFFIC_NOW='2026-08-24 12:00:00' \
SBCTL_CONFIG_DIR="$TMP/period/cfg" \
SBCTL_CONFIG_FILE="$TMP/period/cfg/config.json" \
SBCTL_META_FILE="$TMP/period/meta.json" \
SBCTL_TRAFFIC_FILE="$TMP/period/traffic.json" \
SBCTL_CERT_DIR="$TMP/period/cfg/certs" \
SBCTL_LOG_DIR="$TMP/period/log" \
SBCTL_LOCK_FILE="$TMP/period/lock" \
bash <<'BASH_PERIOD'
set -Eeuo pipefail
source ./sbctl.sh
write_default_config
...
install -m 600 "$tmp" "$TRAFFIC_FILE"; rm -f "$tmp"
SBCTL_TRAFFIC_NOW='2026-09-22 18:29:59'
SBCTL_TRAFFIC_TODAY=2026-09-22
traffic_collect

...
_sbctl_domain_rule_order_filter() {
  cat <<'JQ'
...
def sbctl_ordered_inbound_domains($rules; $inbound):
  [$rules | to_entries[] | select(.value | sbctl_inbound_domain_rule($inbound))]
  | sort_by(
      if (.value | has("domain")) then
        [0, 0, 0, .key]
      else
        (.value.domain_suffix[0]) as $domain |
        [1, (0 - ($domain | split(".") | length)), (0 - ($domain | length)), .key]
      end
    )
  | map(.value);

def sbctl_reorder_inbound_domains($rules; $inbound):
  ([$rules | to_entries[] |
    select(.value | sbctl_inbound_default_rule($inbound)) | .key][0] // null) as $default_index |
  ([$rules | to_entries[] |
    select(.value | sbctl_inbound_domain_rule($inbound)) | .key][0] // null) as $domain_index |
  ($rules | sbctl_ordered_inbound_domains($rules; $inbound)) as $domains |
  if ($domains | length) == 0 then $rules
  elif $default_index != null then
    [range(0; ($rules | length)) as $i |
      if $i == $default_index then
        $domains[], $rules[$i]
      elif ($rules[$i] | sbctl_inbound_domain_rule($inbound)) then empty
      else $rules[$i]
      end]
  elif $domain_index != null then
    [range(0; ($rules | length)) as $i |
      if $i == $domain_index then
        $domains[]
      elif ($rules[$i] | sbctl_inbound_domain_rule($inbound)) then empty
      else $rules[$i]
      end]
...
assign_outbound() {
  ...
  tmp=$(temp_file)
  jq --arg inbound "$inbound" --arg outbound "$outbound" "$(_sbctl_managed_domain_rule_filter)
    $(_sbctl_canonical_default_rule_filter)
    $(_sbctl_domain_rule_order_filter)
    .route = (.route // {}) |
    ...
    .route.rules = sbctl_reorder_inbound_domains(.route.rules; \$inbound)" \
    "$CONFIG_FILE" >"$tmp"
...
add_domain_rule() {
  ...
  jq --arg inbound "$inbound" --arg match "$match" --arg domain "$domain" --arg outbound "$outbound" \
    ...
    def sbctl_current_domain_rule:
      sbctl_managed_domain_rule and .inbound==[\$inbound];
    def sbctl_current_default_rule:
      sbctl_canonical_default_rule and .inbound==[\$inbound];
    ...
    (\$rules | map(sbctl_current_default_rule) | index(true)) as \$default_index |
    (reduce \$rules[] as \$rule
      ({items:[],replaced:false};
       if (\$rule | sbctl_target_domain_rule) then
         if .replaced then . else .items += [\$new_rule] | .replaced=true end
       else .items += [\$rule] end)) as \$inline_state |
    (\$rules | map(select(sbctl_current_domain_rule))) as \$domains |
    (reduce \$domains[] as \$rule
      ({items:[],replaced:false};
       if (\$rule | sbctl_target_domain_rule) then
         if .replaced then . else .items += [\$new_rule] | .replaced=true end
       else .items += [\$rule] end)) as \$domain_state |
    (\$domain_state.items + (if \$domain_state.replaced then [] else [\$new_rule] end)) as \$desired_domains |
    if \$default_index==null then
      .route.rules=(\$inline_state.items + (if \$inline_state.replaced then [] else [\$new_rule] end))
    else
      .route.rules=[range(0; (\$rules|length)) as \$i |
        if \$i==\$default_index then
          (if (\$desired_domains|length)>0 then \$desired_domains[] else empty end),
          \$rules[\$i]
        elif (\$rules[\$i] | sbctl_current_domain_rule) then empty
        else \$rules[\$i]
        end]
    end |
    .route.rules = sbctl_reorder_inbound_domains(.route.rules; \$inbound)" "$CONFIG_FILE" >"$tmp"

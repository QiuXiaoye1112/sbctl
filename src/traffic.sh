...
traffic_limits_show() {
  local rows tag quota used remaining status cycle_start cycle_end feature
  ...
  print_table_cell_clipped "标签" 20; printf '| %12s | %12s | %12s | %8s\n' "月额度" "本周期已用" "剩余" "状态"
  while IFS=$'\t' read -r tag quota used cycle_start cycle_end; do
    ...
    print_table_cell_clipped "$tag" 20
    printf '| %12s | %12s | %12s | %8s\n' "$(traffic_format_bytes "$quota")" "$(traffic_format_bytes "$used")" "$(traffic_format_bytes "$remaining")" "$status"
    printf '  当前周期：%s ～ %s\n' "$cycle_start" "$cycle_end"
  done <<<"$rows"
}

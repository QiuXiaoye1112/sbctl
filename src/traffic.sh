  ...
  if [[ -f $CONFIG_FILE ]]; then
    if [[ -f $META_FILE ]]; then
      jq --slurpfile meta "$META_FILE" '
        ([.inbounds[]? | {key:.tag,value:{protocol:.type,port:.listen_port,disabled:false}}] +
         [($meta[0].disabledInbounds // {})[] | .config |
           {key:.tag,value:{protocol:.type,port:.listen_port,disabled:true}}]) | from_entries
      ' "$CONFIG_FILE" 2>/dev/null || printf '{}\n'
    else
      jq '[.inbounds[]? | {key:.tag,value:{protocol:.type,port:.listen_port,disabled:false}}] | from_entries' "$CONFIG_FILE" 2>/dev/null || printf '{}\n'
    fi
  else
    printf '{}\n'
  fi
...
traffic_period_set() {
  ...
  require_root traffic-period-set
